...
#define SIMPERF_INTERNAL_ASSERT_WINDOW(msg, ...)                                                   \
  MessageBoxA(NULL, ::simperf::Format(msg, __VA_ARGS__).c_str(), "NON-FATAL ASSERT",               \
              MB_OK | MB_ICONWARNING)
#define SIMPERF_DEBUGBREAK(msg, ...) SIMPERF_INTERNAL_ASSERT_WINDOW(msg, __VA_ARGS__)
#else
#define SIMPERF_INTERNAL_ASSERT_WINDOW(msg, ...)                                                   \
  _RPTF0(_CRT_ERROR, ::simperf::Format(msg, __VA_ARGS__).c_str())
#define SIMPERF_DEBUGBREAK(msg, ...)                                                               \
  SIMPERF_INTERNAL_ASSERT_WINDOW(msg, __VA_ARGS__);                                                \
  __debugbreak()
#endif
...
#define SIMPERF_UNPACK_TOKENS(tokens)                                                              \
  [&]() -> std::string {                                                                           \
    ...
    for (const auto val : tokens) {                                                                \
      os << typeid(SIMPERF_GEN_ID(, val)).name() << ' ' << val << ": " << SIMPERF_GEN_ID(, val)    \
         << std::endl;                                                                             \
    }                                                                                              \
    return os.str();                                                                               \
  }()
// need to attach with default error logger
#define SIMPERF_INTERNAL_ASSERT_IMPL(type, check, msg, ...)                                        \
  {                                                                                                \
    if (!(check)) {                                                                                \
      SIMPERF_LOG_ASSERT(msg, __VA_ARGS__);                                                        \
      SIMPERF_DEBUGBREAK(msg, __VA_ARGS__);                                                        \
    }                                                                                              \
  }
#define SIMPERF_INTERNAL_ASSERT_WITH_MSG(type, check, ...)                                         \
  SIMPERF_INTERNAL_ASSERT_IMPL(                                                                    \
      type, check, "Assertion '{0}' failed at {1}:{2}\nCause:{3}", SIMPERF_STRINGIFY_MACRO(check), \
      std::filesystem::path(__FILE__).filename().string(), __LINE__, __VA_ARGS__)
// #define SIMPERF_INTERNAL_ASSERT_NO_MSG(type, check) SIMPERF_INTERNAL_ASSERT_IMPL(type, check,
// "Assertion '{0}' failed at {1}:{2}", SIMPERF_STRINGIFY_MACRO(check),
// std::filesystem::path(__FILE__).filename().string(), __LINE__) //toggle wargs
#define SIMPERF_INTERNAL_ASSERT_NO_MSGW(type, left, right)                                         \
  std::cout << type << left << ' ' << right << std::endl // WARGS

#define SIMPERF_INTERNAL_ASSERT_GET_MACRO_NAME(arg1, arg2, macro, ...) macro
// #define SIMPERF_INTERNAL_ASSERT_GET_MACRO(...) SIMPERF_EXPAND_MACRO(
// SIMPERF_INTERNAL_ASSERT_GET_MACRO_NAME(__VA_ARGS__, SIMPERF_INTERNAL_ASSERT_WITH_MSG,
// SIMPERF_INTERNAL_ASSERT_NO_MSG) ) //toggle wargs
#define SIMPERF_INTERNAL_ASSERT_GET_MACRO(...)                                                     \
  SIMPERF_EXPAND_MACRO(SIMPERF_INTERNAL_ASSERT_GET_MACRO_NAME(                                     \
      __VA_ARGS__, SIMPERF_INTERNAL_ASSERT_WITH_MSG, SIMPERF_INTERNAL_ASSERT_NO_MSGW)) // WARGS

#define SIMPERF_ASSERT(...)                                                                        \
  SIMPERF_EXPAND_MACRO(SIMPERF_INTERNAL_ASSERT_GET_MACRO(__VA_ARGS__)(_, __VA_ARGS__))
#define SIMPERF_ASSERT_WARGS(...)                                                                  \
  SIMPERF_EXPAND_MACRO(SIMPERF_INTERNAL_ASSERT_GET_MACRO(__VA_ARGS__)(_, __VA_ARGS__))
// #define SIMPERF_ASSERT_WARGS(...)
// SIMPERF_UNPACK_TOKENS(SIMPERF_SPLIT_EXPR(SIMPERF_STRINGIFY_MACRO(__VA_ARGS__)))
...
inline bool operator!(register_result rr) { return rr == static_cast<register_result>(0); }
...
public:
  ...
  static const std::string &GetDebugLoggerName(void) { return s_DebugLogger; }

  static const std::string &GetErrorLoggerName(void) { return s_ErrorLogger; }
...
#define SIMPERF_TRACE(logger_name, ...)                                                            \
  SIMPERF_LOG_IT_(logger_name, spdlog::level::trace, __VA_ARGS__)
#define SIMPERF_DEBUG(logger_name, ...)                                                            \
  SIMPERF_LOG_IT_(logger_name, spdlog::level::debug, __VA_ARGS__)
#define SIMPERF_INFO(logger_name, ...)                                                             \
  SIMPERF_LOG_IT_(logger_name, spdlog::level::info, __VA_ARGS__)
#define SIMPERF_WARN(logger_name, ...)                                                             \
  SIMPERF_LOG_IT_(logger_name, spdlog::level::warn, __VA_ARGS__)
#define SIMPERF_ERROR(logger_name, ...)                                                            \
  SIMPERF_LOG_IT_(logger_name, spdlog::level::err, __VA_ARGS__)
#define SIMPERF_CRITICAL(logger_name, ...)                                                         \
  SIMPERF_LOG_IT_(logger_name, spdlog::level::critical, __VA_ARGS__)
